...
if [[ ! -d "$FEATURE_DIR" ]]; then
    echo "ERROR: 기능 디렉터리를 찾을 수 없습니다: $FEATURE_DIR"
    echo "먼저 /specify를 실행하여 기능 구조를 생성하세요."
    exit 1
fi

# Check for implementation plan (required)
if [[ ! -f "$IMPL_PLAN" ]]; then
    echo "ERROR: $FEATURE_DIR에서 plan.md를 찾을 수 없습니다"
    echo "먼저 /plan을 실행하여 계획을 생성하세요."
    exit 1
fi
